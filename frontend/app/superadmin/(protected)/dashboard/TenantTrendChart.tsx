...
export default function TenantTrendChart({ points }: Props) {
  const hasData = points.length > 0

  return (
    <article className="rounded-xl border border-zinc-800 bg-zinc-900/70 p-4">
      <div className="mb-3">
        <h2 className="text-sm font-semibold text-zinc-100">Yeni Tenant Trendi</h2>
        <p className="text-xs text-zinc-400">Son 6 ay yeni tenant sayisi</p>
      </div>
        ...
        <div className="h-[260px]">
          <ResponsiveContainer width="100%" height="100%">
            <LineChart data={points}>
              ...
              />
              <Line type="monotone" dataKey="count" stroke="#4ade80" strokeWidth={2.5} dot={{ r: 3 }} />
            </LineChart>
          </ResponsiveContainer>
        </div>
      )}
